...
// Get all crops belonging to the currently logged-in farmer
exports.getMyCrops = async (req, res) => {
  try {
    // Find the farmer_id of the logged-in user
    const [farmerRows] = await pool.query(
      "SELECT farmer_id FROM farmers WHERE user_id = ?",
      [req.user.user_id]
    );

    // Make sure the user is a registered farmer
    if (farmerRows.length === 0) {
      return res.status(403).json({
        error: "Only registered farmers can view their crops"
      });
    }
    ...
    const [rows] = await pool.query(
      "SELECT * FROM crops WHERE farmer_id = ? ORDER BY crop_id DESC",
      [farmer_id]
    );
